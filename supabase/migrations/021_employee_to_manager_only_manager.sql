-- Migration 021: employee_to_manager allows only manager as target (no hr); avoids duplicate "Colaborador → RH" with employee_to_hr.
-- Run after 020.
...
BEGIN
  ...
  IF v_evaluator_role = 'manager' THEN
    IF p_type IN ('manager_to_employee', 'direct_feedback') AND v_evaluated_role IN ('employee', 'hr', 'manager') THEN
      RETURN;
    END IF;
    RAISE EXCEPTION 'Gestor só pode avaliar colaboradores, outros gestores e RH';
  END IF;

  -- Colaborador: cada tipo tem um alvo específico; direct_feedback permite todos.
  IF v_evaluator_role = 'employee' THEN
    IF p_type = 'employee_to_hr' AND v_evaluated_role = 'hr' THEN RETURN; END IF;
    IF p_type = 'employee_to_manager' AND v_evaluated_role = 'manager' THEN RETURN; END IF;
    IF p_type = 'employee_to_employee' AND v_evaluated_role = 'employee' THEN RETURN; END IF;
    IF p_type = 'direct_feedback' AND v_evaluated_role IN ('manager', 'hr', 'employee') THEN RETURN; END IF;
