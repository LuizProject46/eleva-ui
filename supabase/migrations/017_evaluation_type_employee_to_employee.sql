-- Migration 017: Add evaluation type employee_to_employee (colaborador → colaborador).
-- Run after 016.

-- Add new value to evaluation_type enum (cannot add in middle of enum in PG, new value at end).
ALTER TYPE evaluation_type ADD VALUE IF NOT EXISTS 'employee_to_employee';

-- Allow employee → employee for type employee_to_employee in validate_evaluation_allowed.
...
BEGIN
  ...
  -- Colaborador: employee_to_manager, employee_to_employee, direct_feedback → manager, hr ou employee.
