...
export function renderGenericEmail(params: GenericParams): string {
  const { branding, confirmationUrl, token, email, heading, body, ctaText } = params;
...
<head>
  ...
  <title>${params.subject} - ${branding.companyName}</title>
</head>
          ...
          <tr>
            <td style="padding:40px 40px 24px;">
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" style="margin:0 0 24px;">
                <tr>
                  <td>
                    <a href="${confirmationUrl}" style="display:inline-block;padding:14px 28px;background-color:${branding.primaryColorHex};color:#ffffff;text-decoration:none;font-weight:600;font-size:16px;border-radius:8px;">${ctaText}</a>
                  </td>
                </tr>
              </table>
              ${token ? `<p style="margin:0 0 24px;padding:16px;background:#f4f4f5;border-radius:8px;font-family:monospace;font-size:18px;letter-spacing:4px;color:#18181b;">${token}</p>` : ''}
            </td>
          </tr>
          <tr>
