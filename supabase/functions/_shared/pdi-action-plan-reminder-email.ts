...
export interface PdiActionPlanReminderParams {
  branding: TenantBranding;
  deliveryDateBr: string;
  descriptionSnippet: string;
  pdiUrl: string;
}

export function renderPdiActionPlanReminderEmail(params: PdiActionPlanReminderParams): string {
  const { branding, deliveryDateBr, descriptionSnippet, pdiUrl } = params;
  ...
  const heading = 'Lembrete: plano de ação com data próxima';
  const body = `Um plano de ação do PDI tem entrega prevista para <strong>${deliveryDateBr}</strong>. ${descriptionSnippet} Acesse o portal para acompanhar o plano.`;
          ...
          <tr>
            <td style="padding:40px 40px 24px;">
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" style="margin:0 auto 24px;">
                <tr>
                  <td align="center">
                    <a href="${pdiUrl}" style="display:inline-block;padding:14px 28px;background-color:${branding.primaryColorHex};color:#ffffff;text-decoration:none;font-weight:600;font-size:16px;border-radius:8px;">Acessar o PDI</a>
                  </td>
                </tr>
              </table>
            </td>
